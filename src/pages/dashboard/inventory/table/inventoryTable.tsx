...
function InventoryTable() {
  ...
  const editProduct = (
    <div className='flex'>
      ...
      <img src={imgPrev || productDetails.Image_Url} className="w-16 h-16 object-contain rounded-full" alt="" />
      <CustomInput
        ref={inputRef}
        type="file"
        name='Image_Url'
  ...
  const handleEdit = async() =>{
    if(imgPrev){
      ...
      const productData = { ...productDetails, Image_Url: imageUrl,Last_Updated_By: updatedBy,Last_Updated_On:updatedOn };

      const docRef = doc(db,'inventoryTable',documentId)
      setOpen(false);
      setShowBackdrop(true)
      updateDoc(docRef, productData).then(async()=>{
        const loadedInventory = await fetchInventory('inventoryTable');
        loadedInventory?.shift();
        setInventory(loadedInventory as unknown as inventoryList[]);
      ...
      const updatedOn = new Date().toISOString().slice(0, 19).replace('T', ' ');
      const updatedBy = admin.info?.id;
      const productData = { ...productDetails,Last_Updated_By: updatedBy,Last_Updated_On:updatedOn };
      setOpen(false);
      setShowBackdrop(true)
      updateDoc(docRef, productData).then(async()=>{
        const loadedInventory = await fetchInventory('inventoryTable');
